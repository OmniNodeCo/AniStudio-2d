...
import { canvasBlob, drawShot, framePose, makeCanvas, motionBounds } from "../core/shots";
...
export async function exportGif(
  ...
  const box = motionBounds(scene, from, to);
  ...
  for (let f = from; f <= to; f++) {
    drawShot(ctx, scene, f, { w, h, transparent: o.transparent, overlay: o.overlay, box });
...
export async function exportSpriteSheet(scene: Scene, o: ExportOpts, colsWanted?: number): Promise<SheetResult> {
  ...
  const box = motionBounds(scene, from, to);
  const cw = Math.max(24, Math.round((box.w + 12) * o.scale));
  const ch = Math.max(24, Math.round((box.h + 12) * o.scale));
  ...
  for (let i = 0; i < count; i++) {
    const f = from + i;
    drawShot(cell.ctx, scene, f, { w: cw, h: ch, transparent: o.transparent, overlay: false, box });
    const x = (i % cols) * cw;
...
export async function exportPng(scene: Scene, o: ExportOpts): Promise<Blob> {
  const box = sceneBounds(scene, framePose(scene, 0));
  const { w, h } = shotSize(scene, o, box);
  const { canvas, ctx } = makeCanvas(w, h);
  drawShot(ctx, scene, 0, { w, h, transparent: o.transparent, overlay: o.overlay });
...
export async function exportWebm(scene: Scene, o: ExportOpts, onProgress?: (p: number) => void): Promise<Blob | null> {
  ...
  for (let i = 0; i < frames; i++) {
    drawShot(ctx, scene, from + i, { w, h, transparent: false, overlay: o.overlay, box });
